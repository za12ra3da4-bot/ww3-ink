// 전투 화면 UI — 체력, 탄약, 킬 로그, 미니맵, 점수판, 사망 화면, 결과와 맵 투표, 코인 알림
import { WEAPONS, STREAKS, CLASSES, TEAMS, MODES, MAP_HALF, MAPS } from '../shared/config.js';
import { generateMap } from '../shared/map.js';
import { drawMap, mapThumb } from './assets.js';
import { XH_COLORS } from './settings.js';
...
export function previewMap(id) {
  if (!previewMaps.has(id)) previewMaps.set(id, generateMap(id, 20260914));
  return previewMaps.get(id);
}
...
export class HUD {
  ...
  applySettings(s) {
    ...
    root.setProperty('--xh', XH_COLORS[s.xhColor] || XH_COLORS.ink);
    root.setProperty('--xhk', s.xhSize);
  }

  setMap(map, mode) {
    const c = document.createElement('canvas');
    c.width = c.height = MAP_HALF * 4;
    drawMap(c.getContext('2d'), map, c.width, { points: false, spawns: false });
    this.staticMap = c;
    ...
    const M = MAPS.find((m) => m.id === map.id);
    $('goal').textContent = `${M ? M.name : ''} · 목표 ${MODES[mode].scoreLimit}`;
  }
  ...
  streaks(p) {
    const me = this.g.roster.get(this.g.me.nid), st = me ? me.st : p.streak || 0;
    p.streak = st;
  ...
  crosshair(p, ads, scoped) {
    ...
    const px = (ads ? 2 : 4) + (spread * (window.innerHeight / 2)) / Math.tan((p.fov * Math.PI) / 360);
    const ch = $('crosshair');
    ch.style.setProperty('--s', `${px.toFixed(1)}px`);
    ch.classList.toggle('ads', ads);
    ch.classList.toggle('hide', scoped || p.sprinting || !p.alive);
    $('scope').hidden = !scoped;
  }
  ...
  coins(n, why) {
    ...
    el.innerHTML = `<img src="assets/ui/coin.svg" alt=""><b>+${n}</b><span>${esc(why)}</span>`;
    box.appendChild(el);
  ...
  announce(text, sub = '', tone = '') {
    ...
    el.className = tone === true ? 'red' : tone || '';
    void el.offsetWidth;
    el.classList.add('show');
  }
  ...
  top() {
    ...
    g.cp.forEach(([owner, prog, contested], i) => {
      ...
      el.firstChild.style.background = prog < 0 ? TEAMS[0].css : TEAMS[1].css;
    });
  }
  ...
  minimap() {
    ...
    if (g.mode === 'dom' && g.map) {
      g.map.points.forEach((P, i) => {
        ...
        ctx.fillStyle = owner === 0 ? 'rgba(21,20,18,.4)' : owner === 1 ? 'rgba(44,93,143,.45)' : 'rgba(21,20,18,.12)';
        ctx.fill();
        ctx.save();
        ctx.translate(P.x, P.z);
        ctx.rotate(-yaw);
        ctx.fillStyle = owner === 1 ? '#1d4670' : '#151412';
    ...
    for (const sd of g.soldiers.values()) {
      ...
      ctx.beginPath();
      ctx.arc(st.x, st.z, mate ? 1.7 : 2.2, 0, Math.PI * 2);
      ctx.fillStyle = mate ? TEAMS[sd.team].css : '#c0301a';
      ctx.fill();
      ctx.lineWidth = 0.6;
    ...
    ctx.restore();
    if (uav) {
      ctx.strokeStyle = 'rgba(192,48,26,.7)';
      ctx.lineWidth = 3;
      ctx.beginPath();
      ctx.arc(R, R, R - 3, -Math.PI / 2, -Math.PI / 2 + ((g.time * 2) % (Math.PI * 2)));
      ctx.stroke();
    }
    ctx.fillStyle = TEAMS[g.me.team].css;
    ctx.beginPath();
  ...
  scoreboard(show) {
    ...
    el.innerHTML = `<div class="sb-grid">${teams.map((list, t) => `
      ...
      ${list.map((r) => {
        ...
        return `<tr class="${r.nid === g.me.nid ? 'me' : ''}${dead ? ' dead' : ''}"><td>${esc(r.name)}${r.bot ? '<span class="tag">AI</span>' : ''}</td><td>${CLS[r.cls] ? CLS[r.cls].name : ''}</td><td>${r.k}</td><td>${r.d}</td><td>${r.a}</td><td>${r.sc}</td></tr>`;
      }).join('')}</table></div>`).join('')}</div>
      <p class="sb-foot">방 코드 <b>${g.code}</b> · ${MODES[g.mode].name}</p>`;
  }
  ...
  classCards(el, current, onPick) {
    ...
    const t = $('endTitle');
    t.textContent = m.waiting ? '待' : draw ? '和' : win ? '勝' : '敗';
    t.className = `hanja${win ? ' win' : ''}`;
    $('endSub').textContent = m.waiting ? '다음 전투를 준비하는 중입니다'
      : `${TEAMS[0].name} ${m.scores[0]} : ${m.scores[1]} ${TEAMS[1].name}` + (draw ? ' — 무승부' : ` — ${TEAMS[m.winner].name} 승리`);
    const top = m.waiting ? [] : [...m.roster].sort((a, b) => b.sc - a.sc).slice(0, 6);
    $('endTable').innerHTML = !top.length ? '' : `<div class="sb-team"><table><tr><th>순위</th><th>이름</th><th>처치</th><th>전사</th><th>도움</th><th>점수</th></tr>${top.map((r, i) =>
    ...
    grid.innerHTML = MAPS.map((M, i) => `
      <button class="vote-card" data-map="${M.id}">
        <img src="${mapThumb(previewMap(M.id), 200)}" alt="">
        <div class="vc-name"><b class="hanja">${M.hanja}</b>${M.name}</div>
        <div class="vc-bar"><i></i></div><div class="vc-count">0표</div><kbd>${i + 1}</kbd>
      </button>`).join('');
    grid.querySelectorAll('.vote-card').forEach((b) => b.addEventListener('click', () => g.vote(b.dataset.map)));
    if (m.vote) this.votes(m.vote);
    $('endScreen').hidden = false;
  }

  votes(info) {
    const total = Object.values(info.counts).reduce((a, b) => a + b, 0) || 1;
    document.querySelectorAll('.vote-card').forEach((c) => {
      const n = info.counts[c.dataset.map] || 0;
      c.querySelector('.vc-bar i').style.width = `${(n / total) * 100}%`;
      c.querySelector('.vc-count').textContent = `${n}표`;
      c.classList.toggle('mine', this.myVote === c.dataset.map);
    });
    this.g.voteEndsAt = this.g.time + info.left;
  }
  ...
  frame() {
    ...
    if (!$('deathScreen').hidden) $('respawnTimer').textContent = g.respawnAt > t ? `${Math.ceil(g.respawnAt - t)}초 후 출진 · 1~4 키 또는 클릭으로 병과 선택` : '곧 출진';
    if (g.nuke) {
      const left = Math.ceil(g.nuke.at - t);
      if (left !== this.nukeLeft && left >= 0) {
        this.nukeLeft = left;
        this.announce(`전술핵 ${left}`, g.nuke.team === g.me.team ? '아군 핵 투하 대기' : '적의 핵이 떨어진다 — 숨을 곳은 없다', 'red');
      }
    }
    if (!$('endScreen').hidden) $('endNext').textContent = `${Math.max(0, Math.ceil((g.voteEndsAt || 0) - t))}초 후 다음 전투 · 다음 전장에 투표하세요 (클릭 또는 1~5)`;
  }
}
